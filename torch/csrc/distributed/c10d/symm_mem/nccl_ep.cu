...
c10::intrusive_ptr<NcclEpHandle> nccl_ep_create_handle(
    ...
    // HT FLAT metadata writes per-expert recv counts.
    // Always bind a live int32 buffer (caller-owned or
    // internally allocated) so we stay on the illegal-address
    // in the nopec scan JIT.
    ...
    if (recv_expert_counter.has_value()) {
      ...
      expert_counter = at::zeros(
          ...
          topk_idx.oiptions().dtype(at::kInt));
    }
